...
class LoRALinear(LoRALayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        This class has 4 weight matrices:
            1. Pretrained weights are stored as `self.linear.weight`
            2. LoRA A matrix as `self.lora_A`
            3. LoRA B matrix as `self.lora_B`
            4. LoRA B matrix as `self.lora_C`
        Only LoRA's C matrix is updated, other weights stay frozen.
        ...
        if r > 0:
            ...
            self.lora_C = nn.Parameter(torch.zeros((r, r)))
    ...
    def reset_parameters(self) -> None:
        ...
        if hasattr(self, "lora_C"):
            # initialize A & B the same way as the default for nn.Linear and C to zero
    ...
    def merge(self) -> None:
        ...
        if self.r > 0 and not self.merged:
            ...
            self.linear.weight.data += (
                self.lora_B @ self.lora_C @ self.lora_A
            ) * self.scaling
            self.merged = True
...
class LoRAQKVLinear(LoRALinear):
        ...
        **kwargs,
        ...
        if r > 0 and any(enable_lora):
            ...
            self.lora_C = nn.Parameter(
                torch.zeros((r * sum(enable_lora), r * sum(enable_lora)))
            )  # (4, 4)
            ...
            # - self.lora_C has shape (4, 4): 4 because  rank is 2 and LoRA is applied only to two matrices
    ...
    def merge(self) -> None:
        ...
        # ⚬ self.lora_C.data: (4, 4)
        if self.r > 0 and any(self.enable_lora) and not self.merged:
            delta_w = self.conv1d(
                self.lora_A.data.unsqueeze(0),  # (4, 128) -> (1, 4, 128)
                self.lora_C.data.unsqueeze(-1),  # (4, 4) -> (4, 4, 1)
            )  # (1, 4, 128) @ (4, 4, 1) -> (1, 4, 128)
            delta_w = self.conv1d(
                delta_w,  # (1, 4, 128)
                self.lora_B.data.unsqueeze(-1),  # (256, 2) -> (256, 2, 1)
            )  # (1, 4, 128) @ (256, 2, 1) -> (1, 256, 128)
            delta_w = delta_w.squeeze(0)  # (1, 256, 128) -> (256, 128)
            # W = W + delta_W (merge)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        after_B = self.conv1d(
            after_C.transpose(-2, -1),  # (64, 64, 4) -> (64, 4, 64)
...
def mark_only_lora_as_trainable(model: nn.Module, bias: str = "none") -> None:
    ...
    """
    # freeze all layers except LoRA-C's
...
class Config(BaseConfig):
    ...
    def mlp_class(self) -> Type:
        return getattr(lit_gpt.lorac, self._mlp_class)
